...
exports.forgotPassword = async (req, res) => {
  ...
  if (!_b.college_id)
    ...
    try {
      const user = await User.findOne({
        college_id: req.body.college_id.toUpperCase(),
      }).select("college_id college_email token student_name");
...
exports.verifyToken = async (req, res) => {
  ...
  if (!_b.token) {
    ...
    try {
      const user = await User.findOne({ token: _b.token }).select(
        "college_id token"
      );
...
exports.resetPassword = async (req, res) => {
  ...
  if (!_b.password || !_b.token) {
    ...
    try {
      ...
      const user = await User.findOne({ token: _b.token }).select(
        "_id college_id student_name college_email password token"
      );
...
exports.profile = (req, res) => {
  User.findOne({ college_id: req.decoded.college_id })
    ...
    .then((profile) => {
      ...
      profile.cgpa = profile.cgpa ? profile.cgpa.toString() : "";
      profile.matric_marks = profile.matric_marks
        ? profile.matric_marks.toString()
        : "";
      profile.senior_marks = profile.senior_marks
        ? profile.senior_marks.toString()
        : "";
