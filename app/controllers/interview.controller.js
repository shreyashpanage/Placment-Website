...
exports.getAll = (req, res) => {
  ...
  Interview.find({ status: "approved" })
...
exports.getOne = (req, res) => {
  ...
  Interview.findOne({ _id: _b.experience_id })
    .lean()
    .then((experience) => {
  ...
  Interview.create({
    ...
    .then((data) => {
      res.status(200).json({
        success: true,
        message:
          "Thanks for your contribution! Sit back and relax while our reviewers approves your interview experience.",
      });
    })
    .catch((err) => {
      console.error(err);
...
exports.changeStatus = async (req, res) => {
  ...
  try {
    const interview = await Interview.findOne({ _id: _b.experience_id }).select(
      "status author_id author_name title"
    );
    ...
    if (interview.status === "pending") {
      ...
      res.status(200).json({
        success: true,
        message:
          "Interview experience han been updated. Author will be notified.",
      });

      const notifyAuthor = await Mailer.sendDM(
        interview,
...
exports.edit = (req, res) => {
  ...
  Interview.updateOne({ _id: _b._id }, _b)
    .then((data) => {
      res.status(200).json({
        success: true,
