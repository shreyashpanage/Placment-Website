...
exports.getStatus = (req, res) => {
  ...
  Company.findOne({ _id: _b.company_id })
    ...
    .lean()
    .then((company) => {
      let isCandidateAlreadyRegistered = company.candidates.find(function (
        candidate
      ) {
...
exports.oneClickApply = async (req, res) => {
  ...
  try {
    const redFlagHistory = await RedFlag.find({
      receiver: req.decoded.college_id,
      active: true,
    });
    ...
    if (redFlags >= 3) {
      res.status(200).json({
        success: false,
        message:
          "Your profile has been blocked by Placement Cell due to more than 3 red flags.",
      });
    } else {
      ...
      const isCandidateAlreadyRegistered = company.candidates.find(function (
        ...
        console.log("candidate.college_id--->", req.decoded.college_id);
        return candidate.college_id == req.decoded.college_id;
      });

      if (!isCandidateAlreadyRegistered) {
        // todo = Validations
        const id = parseInt(req.decoded.college_id);
        console.log("id---->", id, "req.dec--->", req.decoded.college_id);
...
exports.withdraw = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Registration successfully withdraw." });
  } catch (err) {
    console.error(err);
    res.status(200).json({ success: false, message: "Something went wrong!" });
  }
